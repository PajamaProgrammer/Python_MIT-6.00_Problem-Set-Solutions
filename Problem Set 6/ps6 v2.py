...
#
# Updated 23-Feb-2016 - Version 1 of generate_combinations() helper function never sat right with me as being very
# optimal time-complexity wise. Version 1 uses recursion and as such wastes a lot of time doing over-lapping sub-function calls.
# I updated this function to utilize a binary trick to generate all possible combinations.
# For example, if there are 7 letters, then there are 2^7 = 128 possible combinations.
# If I convert all the numbers from the range of 1 to 2^(the current hand size) into binary.
# I can then map each binary number to a letter combination in the hand; if '1', include letter in combo, if '0' throw out letter...
# For the example of 'abc':
##1 001 c
##2 010 b
##3 011 bc
##4 100 a
...
##6 110 ab
##7 111 abc
# Version 2 method is even faster! Average time of using pick_best_word_faster(hand) is now 0.01 seconds on my machine!
...
def generate_combinations(string): #version 2
    """
    ...
    for i in range(1,pow(2,len(A))):
        bIndex = bin(i)[2:].zfill(len(A)) #permutation index will go from 1 - 128 (if full hand size) (in binary 000 0001 to 111 1111); this is used to help generate all possible permutations
        #print(i, pIndex)
        perm = ""
        index = 0
        for b in bIndex:
...
##def generate_combinations(string): #version 1
##    """
##    #Helper function I wrote, it accepts a string and will generate every possible subset of that string
##    """
##    A = string
